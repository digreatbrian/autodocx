...
if t.TYPE_CHECKING:
    from autodocx.contrib.utils import ItemData
...
class MystRenderer(RendererBase):
    ...
    def generate_summary(
        ...
        for item in objects:
            ...
            if self.show_docstring(item):
                yield f"  - ```{{autodocx-docstring}} {full_name}"
    ...
    def render_package(self, item: ItemData) -> t.Iterable[str]:
        ...
        if self.show_docstring(item):
            yield f"```{{autodocx-docstring}} {item['full_name']}"
    ...
    def render_class(self, item: ItemData) -> t.Iterable[str]:
        ...
        if self.show_docstring(item):
            lines.append(f"```{{autodocx-docstring}} {item['full_name']}")
            if parser_name := self.get_doc_parser(item["full_name"]):
                ...
                if init_item:
                    lines.extend(
                        [
                            ...
                            "```",
                            "",
                            f"```{{autodocx-docstring}} {init_item['full_name']}",
                        ]
                    )
