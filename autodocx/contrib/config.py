"""
The configuration module for the extension.
"""
...
if t.TYPE_CHECKING:
    from autodocx.contrib.render.base import RendererBase
...
def _load_renderer(name: str, item: t.Any) -> type[RendererBase]:
    ...
    if item in ("rst", "restructuredtext"):
        from autodocx.contrib.render.rst_ import RstRenderer
        return RstRenderer
    
    if item in ("markdown", "md", "myst"):
        from autodocx.contrib.render.myst_ import MystRenderer
        return MystRenderer
...
class Config:
    ...
    index_template: str | None = dc.field(
        default=(
            ...
            "{%- endfor %}\n"
            "\n"
            ".. [#f1] Created with `autodocx <https://github.com/digreatbrian/autodocx>`_\n"
            "\n"
        ),
        metadata={
            "help": (
