...
from autodocx.contrib.config import CONFIG_PREFIX, Config, ValidationError
from autodocx.contrib.db import Database, InMemoryDb
from autodocx.contrib.resolve_all import AllResolver
from autodocx.contrib.utils import WarningSubtypes

if t.TYPE_CHECKING:
    from docutils.parsers.rst.states import RSTStateMachine

LOGGER = logging.getLogger("autodocx")
...
def get_database(env: BuildEnvironment) -> Database:
    ...
    if not hasattr(env, "autodocx_db"):
        env.autodocx_db = InMemoryDb()  # type: ignore
    return env.autodocx_db  # type: ignore
...
def get_all_analyser(env: BuildEnvironment) -> AllResolver:
    ...
    # We store it persistently in the environment so that it can be
    # accessed by directives and other extensions.
    if not hasattr(env, "autodocx_all_analyser"):
        env.autodocx_all_analyser = AllResolver(get_database(env), _warn)  # type: ignore
    return env.autodocx_all_analyser  # type: ignore
